...
const data = location.state 
  ...
  function handleMultipleChange(e) {
    if (e.target.files.length > 0) {
        ...
        render.onloadend = () => {
          //console.log(render.result.toString());
          let newFile = [...files];
          newFile.push(render.result.toString());
          console.log(newFile)
         setFiles(newFile);
        };
  ...
  console.log(files)
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Handle file URLs (e.g., display them or save them as needed)
  ...
  // Update functions for fetching categories and subcategories
  const getCategories = (superCategoryId) => {
    const superCategoryData = productData.find(data => data._id === superCategoryId);
    return superCategoryData ? superCategoryData.categories : [];
  };
    ...
    <form onSubmit={handleSubmit} >
      ...
      <div>
        ...
        <select value={origin} onChange={(e) => setOrigin(e.target.value)}>
          ...
          {/* Replace `countries` with your actual list of countries }
      ...
      <div>
        <label>Product ID Sufix</label>
        <input type="text" value={data.info.id} readOnly />
      </div>
